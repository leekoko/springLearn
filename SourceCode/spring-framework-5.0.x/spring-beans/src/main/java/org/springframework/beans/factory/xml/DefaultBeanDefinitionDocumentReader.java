...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	public void registerBeanDefinitions(Document doc, XmlReaderContext readerContext) {
		...
		//获取Document的根元素<beans>标签
		Element root = doc.getDocumentElement();
		//实现BeanDefinition的解析和注册工作
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		//将BeanDefinition解析工作交给BeanDefinitionParserDelegate去完成   委托模式
		BeanDefinitionParserDelegate parent = this.delegate;
		this.delegate = createDelegate(getReaderContext(), root, parent);
		//判断该标签是否包含spring的默认命名空间
	...
	protected void parseBeanDefinitions(Element root, BeanDefinitionParserDelegate delegate) {
			//加载Document对象是否使用了Spring默认的XML命名空间
			if (delegate.isDefaultNamespace(root)) {
				...
				for (int i = 0; i < nl.getLength(); i++) {
					...
					if (node instanceof Element) {
						...
						//bean标签、import标签、alias标签，使用默认解析规则
						if (delegate.isDefaultNamespace(ele)) {
							parseDefaultElement(ele, delegate);
						}
						else { //context标签、aop标签、tx标签，则使用自定义的解析规则解析元素节点
